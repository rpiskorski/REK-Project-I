...
#     test = interactions_df_test.groupby(by='user_id').count().sort_values(by="term")
#     display(test)

    test_user_interactions = interactions_df_test.groupby(by='user_id')
#     test_user_interactions.apply(print)
    # MAIN
    # print(("GROUP: ",interactions_df_test.columns))

    recommendations_total = recommender.recommend(pd.DataFrame(interactions_df_test.loc[:,'user_id'].unique().tolist(), columns=['user_id']),items_df, n_recommendations=10)
    # MAIN
...
#                                                 items_df, n_recommendations=10)
        recommendations = recommendations_total.loc[recommendations_total['user_id']==user_id].sort_values(by="score",ascending=False)
